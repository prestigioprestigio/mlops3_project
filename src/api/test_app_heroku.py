...
def test_root():
    r = requests.get("https://mlops3-app.herokuapp.com/")
...
def test_post_true(data_true_json):
    r = requests.post("https://mlops3-app.herokuapp.com/infer", data=json.dumps(data_true_json))
    assert r.status_code == 200
    assert r.json()['result'] == True

    
def test_post_false(data_false_json):
    r = requests.post("https://mlops3-app.herokuapp.com/infer", data=json.dumps(data_false_json))
    assert r.status_code == 200
